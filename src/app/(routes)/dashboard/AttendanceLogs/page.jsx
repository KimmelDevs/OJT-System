...
import { DownloadIcon, AlertCircleIcon, BookOpenIcon, ClockIcon } from 'lucide-react';
...
export default function AttendanceLogs() {
  ...
  const fetchJournalEntries = async (userId) => {
    ...
    for (const doc of attendanceSnapshot.docs) {
      const journalRef = collection(db, 'users', userId, 'attendance', doc.id, 'journal');
  ...
  }, [timeFilter, activeTab, currentUser]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#22201a' }}>
        <div className="text-center">
          ...
          <p className="mt-4" style={{ color: '#fefffe' }}>Loading records...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-8" style={{ backgroundColor: '#22201a' }}>
      <Card 
        title={activeTab === 'attendance' ? "Attendance Logs" : "Daily Journals"} 
