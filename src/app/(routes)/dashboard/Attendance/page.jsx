...
import { ClockIcon, CheckIcon, AlertCircleIcon, BookOpenIcon } from 'lucide-react';
...
export default function DailyAttendance() {
  ...
  const saveJournalEntry = async () => {
    if (!currentUser || !journalContent.trim()) return;
    setIsSubmitting(true);
    
    try {
      const journalRef = collection(db, 'users', currentUser.uid, 'attendance', dateId, 'journal');
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleJournalSubmit}>
          <div className="mb-4">
            <label htmlFor="journal-title" className="block text-sm font-medium mb-1" style={{ color: '#fefffe' }}>
              Title (Optional)
            </label>
            <input
              id="journal-title"
          ...
          <div className="mb-4">
            <label htmlFor="journal-content" className="block text-sm font-medium mb-1" style={{ color: '#fefffe' }}>
              Today's Notes
            </label>
            <textarea
              id="journal-content"
              ...
              className="w-full px-3 py-2 rounded-lg focus:outline-none"
              placeholder="What did you work on today? Any accomplishments or challenges?"
            />
          </div>
